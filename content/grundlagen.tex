...
\begin{document}
	...
	\begin{proof}
		...
		$z = \varphi(y) \in \varphi(X)$ impliziert.
	\end{proof}
	...
	\begin{proof}
		...
		$A_n \defby U_n^{\mathsf{c}}$ schreiben. Ferner sei 
		...
		$x \in A_m$. Damit gilt aber für alle $k \in \N$
		$$\sup_{l \geq k} d^\ast(x_k, x_l) \; \geq \; \sup_{l \geq k} 
			\, \sum_{n=1}^{\infty} \min \left(
			\frac{1}{2^n}, \left| \frac{1}{d(x_k, A_n)} -
			 \frac{1}{d(x_l, A_n)} \right|
			\right) \; \geq \; \frac{1}{2^m} \; > \; 0 \text{,}$$
		was der Tatsache widerspricht, dass $(x_k)_k$ eine Cauchyfolge 
		bezüglich $d^\ast$ ist. Damit muss also $x \in P$ gelten. 
		Aus der Stetigkeit von $x \mapsto d(x, A_n)$ folgt mit dem 
	...
	\begin{Bemerkung}
		Da eine Teilmenge von $H$ genau dann kompakt ist, wenn sie 
		abgeschlossen (und damit nach Hilfssatz~\ref{lem:opensets} 
		eine $G_\delta$-Teilmenge) ist, entsprechen die kompakten 
		polnischen Räume bis auf Homöomorphie den abgeschlossenen 
		Teilmengen von $H$.
	\end{Bemerkung}
