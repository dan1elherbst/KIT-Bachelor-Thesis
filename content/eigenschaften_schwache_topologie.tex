...
\begin{document}
	...
	\begin{proof}
		...
		Offenbar ist $l$ wohndefiniert und linear. Da $l(f) \leq C \norm{f}_\infty$ für alle $f \in \Bdcontfct{\X}$ gilt, folgt $l \in A^\ast$. Nach dem Satz von Hahn-Banach (vgl. \cite[Corollary 5.5.2]{Simon.2015})
	...
	\begin{proof}
		...
		mit der Schwach-$\ast$-Topologie. Mit dem Satz von Banach-Alaoglu (\cite[Theorem 5.8.1]{Simon.2015}) folgt nun, dass $A$ als abgeschlossene Teilmenge des Kompaktums 
	...
	\begin{Satz}[Prokhorov]
		...
		Sei $\X$ ein metrisierbarer topologischer Raum und $A \subseteq \Probmeasures{\X}$. Dann gelten die folgenden Aussagen.
		\begin{enumeratethm}
			\item Ist $\X$ separabel und $A \subseteq \Probmeasures{X}$ straff, so ist $\overline{A} \subseteq \Probmeasures{\X}$ kompakt.
			\item Ist $\X$ polnisch und $\overline{A} \subseteq \Probmeasures{\X}$ kompakt, so ist $A \subseteq \Probmeasures{X}$ straff.
		\end{enumeratethm}
	\end{Satz}

	\begin{Bemerkung}
		Meistens wird Satz~\ref{satz:prokhorov} in der folgenden Form verwendet: Ist $\X$ polnisch und $A \subseteq \Probmeasures{\X}$, so gilt
		\[ A \text{ ist straff} \quad \iff \quad \overline{A} \text{ ist kompakt} \text{.} \]
	...
	\begin{proof}
		Sei zunächst $\X$ separabel und $A \subseteq \Probmeasures{\X}$ straff. Wir möchten nun beweisen, dass $\overline{A}$ 
		...
		Sei nun $\X$ polnisch und $\overline{A}$ kompakt. Außerdem sei $d$ eine Metrik, die $\X$ vollständig metrisiert 
		...
		\]
		und damit ist $A$ straff. Also folgt auch Aussage (b).
	\end{proof}
