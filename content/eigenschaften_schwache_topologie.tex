...
\begin{document}
	...
	Nachdem wir im vorigen Abschnitt die schwache Topologie definiert haben, möchten wir uns nun einige Eigenschaften dieser ansehen, wenn man weitere Bedingungen an den Grundraum $X$ stellt. Unsere zentralen Erkenntnisse werden die folgenden Aussagen sein:
	\begin{itemize}
		\item Ist $X$ kompakt und metrisierbar, so ist $\Probmeasures{X}$ kompakt.
		\item Ist $X$ separabel und metrisierbar, so ist auch $\Finitemeasures{X}$ separabel und metrisierbar.
		\item Ist $X$ polnisch, so ist auch $\Finitemeasures{X}$ polnisch.
	\end{itemize}
	...
	Wir beginnen mit der ersten Aussage, dessen Beweis die Charakterisierung der schwachen Topologie als Schwach-$\ast$-Topologie aus Abschnitt~\ref{subsec:BeziehungSchwachStern} verwendet.
	...
	Wir werden uns nun noch der dritten Aussage vom Beginn dieses Abschnittes widmen. Diese zeigen wir zunächst für den wichtigen Spezialfall, dass der Grundraum $X$ kompakt ist und weiten sie anschließend auf die Klasse aller polnischen Räume aus.
	...
	\begin{Bemerkung}
		Da $\Probmeasures{X} \subseteq \Finitemeasures{X}$ eine abgeschlossene Teilmenge ist, gelten die Aussagen von Satz~\ref{thm:finitemeasuresmetrizableseparable} und Satz~\ref{thm:finitemeasurespolish} auch für $\Probmeasures{X}$.
	\end{Bemerkung}
