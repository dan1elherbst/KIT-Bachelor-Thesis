...
\begin{document}
	...
	Im zugehörigen Beweis haben wir allerdings explizit gar keine Metrik angegeben, die $\Probmeasures{\X}$ vollständig metrisiert. Analysiert man den Beweis schrittweise, so lässt sich die Metrik $D$,
	...
	Offenbar ist diese Metrik kompliziert und unhandlich, denn $\X$ muss etwa selbst mit einer recht speziellen Metrik versehen sein. Zudem ist die Metrik in dem Sinne etwas unintuitiv, dass es völlig unklar bleibt, 
	...
	Zunächst werden wir in Teilabschnitt~\ref{subsec:kopplungen} und \ref{subsec:optimaler-transport} einige grundlegende Denkweisen, Begriffe und Ergebnisse aus der Theorie des optimalen Transports vorstellen,
	die wir anschließend in Teilabschnitt~\ref{subsec:wassersteinmetriken} verwenden, um die Wassersteinmetriken zu beschreiben. Dieser Abschnitt orientiert sich an \cite[Kapitel 4-6]{Villani.2009}.
	
	\subsection{Kopplungen}
	\label{subsec:kopplungen}
	
	Im optimalen Transport modellieren Wahrscheinlichkeitsmaße nicht den Zufall, sondern werden als räumliche Verteilungen von Gütern (oder allgemeiner Masse) angesehen. Die Ausgangssituation können wir uns wie folgt vorstellen:
	Wir nehmen an, dass wir eine gewisse Menge an Erde aus dem Boden fördern müssen, um diese anschließend an Stellen zu bringen, an denen sie weiterverarbeitet werden kann. Die räumlichen Verteilung vor und
	nach dem Transport sind hierbei festgelegt und werden jeweils durch (Borel-)Wahrscheinlichkeitsmaße $\mu$ bzw. $\nu$ auf Räumen $\X$ bzw. $\Y$ modelliert.
	...
	\begin{Satz}
		...
		Sei $(\X, d)$ ein polnischer metrischer Raum, wobei $d$ eine beschränkte Metrik sei, und sei $p \in [1, \infty)$. Dann ist $(\Probmeasures{\X}, W_p)$ auch ein polnischer metrischer Raum.
	\end{Satz}
