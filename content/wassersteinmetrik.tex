...
\begin{document}
	...
	\section{Wassersteinmetriken und optimaler Transport}
	
	Einer unserer zentralen vorigen Erkenntnisse war, dass für einen polnischen Raum $\X$ auch $\Finitemeasures{\X}$ bzw. $\Probmeasures{\X}$ polnisch ist (siehe Satz~\ref{thm:finitemeasurespolish}). 
	...
	die uns in Satz~\ref{thm:finitemeasurespolish} zu unserer Erkenntnis verhalf, allerdings rekonstruieren: Es gilt nämlich
	...
	Zunächst werden wir in den Teilabschnitten~\ref{subsec:kopplungen} und \ref{subsec:optimaler-transport} einige grundlegende Denkweisen, Begriffe und Ergebnisse aus der Theorie des optimalen Transports vorstellen,
	die wir anschließend in Teilabschnitt~\ref{subsec:wassersteinmetriken} verwenden, um die Wassersteinmetriken zu beschreiben. Dieser Abschnitt orientiert sich überwiegend an \cite[Kapitel 4-6]{Villani.2009}.
	...
	\label{subsec:optimaler-transport}
	...
	\begin{Bemerkung}
		Der Beweis folgt \cite[Theorem 4.1 und Lemma 4.3]{Villani.2009}, wo eine allgemeinere Version des Satzes vorgestellt wird, die lediglich die Unterhalbstetigkeit von $c$ fordert 
	...
	Für den Beweis von Satz~\ref{thm:existenceoptimaltransportplan} ist noch ein Hilfssatz von Nöten.
	...
	Mit der Ergebnissen aus den Teilabschnitten~\ref{subsec:kopplungen} und \ref{subsec:optimaler-transport} sind wir nun bereit, die Wassersteinmetriken $W_p$ für $p \in [1, \infty)$ zu definieren. Der wesentliche Gedanke hierbei ist,
	...
	In diesem Teilabschnitt werden wir mit Satz~\ref{thm:wassersteinweakconvergence} das zentrale Resultat dieses Abschnitts formulieren und beweisen: Unter gewissen Voraussetzungen metrisiert $W_p$ die schwache Topologie und $(\Probmeasures{X}, W_p)$ ist selbst ein polnischer metrischer Raum.
	...
	\begin{Bemerkung}
		...
		ist also eine isometrische Einbettung. Insbesondere ist dies auch vor dem Kontext von Satz~\ref{thm:embeddingdiracmeasures} zu betrachten, wo wir nachgewiesen haben, dass obige Abbildung ein Homöomorphismus auf 
	...
	Auch abgesehen davon, dass es sich bei $W_p$ streng genommen noch um keine Metrik handelt, metrisiert $W_p$ die schwache Konvergenz für jeden polnischen metrischen Raum $(\X, d)$ noch nicht.
	In \cite{Villani.2009} wird dies umgangen, indem $\Probmeasures{\X}$ auf eine Teilmenge eingeschränkt wird, auf der $W_p$ lediglich endliche endliche Werte annimmt, und ein leicht abgeändertes Konzept
	...
	Wir sind nun bereit, uns dem zentralen Satz dieses Abschnittes zu widmen.
	...
	\begin{Satz}
		...
		Topologie und $(\Probmeasures{\X}, W_p)$ auch ein polnischer metrischer Raum.
	\end{Satz}

	Der Beweis orientiert sich an \cite[Theorem 6.9 und Korollar 6.13]{Villani.2009}, wobei sich der Aufwand durch die Annahme der Beschränktheit von $d$ stellenweise verringert.
	Außerdem ist noch ein letzter Hilfssatz von Nöten, in dem die obige Dualitätsformel eingeht.
	...
	\begin{proof}[Beweis von Satz~\ref{thm:wassersteinweakconvergence}]
		Wegen Satz~\ref{thm:finitemeasurespolish} ist lediglich zu beweisen, dass $\Probmeasures{X}$ vollständig durch $W_p$ metrisiert wird. 
		...
		Für beide Richtungen verwenden die elementare Tatsache, dass für $(\mu_k)_k \in \Probmeasures{\X}^\N$ und $\mu \in \Probmeasures{\X}$ die Äquivalenz
		\[ \begin{matrix}
		...
		Es bleibt zu zeigen, dass $(\Probmeasures{\X}, W_p)$ vollständig ist. Sei dazu $(\mu_k)_k$ eine Cauchyfolge in $\Probmeasures{\X}$ bezüglich $W_p$. Nach Hilfssatz~\ref{lem:wassersteincauchyseq} ist 
