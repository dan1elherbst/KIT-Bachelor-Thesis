...
\begin{document}
	...
	\begin{tikzpicture}[define rgb/.code={\definecolor{mycolor}{RGB}{#1}},
		...
		\begin{axis}[xmax=5, ymax=1, xmin=-5, ymin=0, samples=200]
			\foreach \t in {0,...,\steps} 
			{
				\def\mu{\startmu + \t * (\endmu - \startmu) / \steps}
				\def\sigma{\startsigma + \t * (\endsigma - \startsigma) / \steps}
				%\def\r{\startr + \t * (\endr - \startr) / \steps}
				%\def\g{\startg + \t * (\endg - \startg) / \steps}
				%\def\b{\startb + \t * (\endb - \startb) / \steps}
				\addplot[rgb color={0, 156, 129}, thick]  {gauss(x, \mu, \sigma)};
			}
		\end{axis}
	\end{tikzpicture}
