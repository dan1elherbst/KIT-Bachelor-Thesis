...
\begin{document}
	...
	\begin{proof}
		...
		Durch eine Anwendung des Satzes von Prokhorov (Satz~\ref{thm:prokhorov}) können wir aus (a) und (b) unmittelbar auch Aussage (c) folgern, denn sowohl in $\Probmeasures{X}$ und $\Probmeasures{Y}$ als auch in $\Probmeasures{X \times Y}$ entsprechen kompakte Teilmengen gerade den Teilmengen, die straff und abgeschlossen sind.
	\end{proof}
	...
	\begin{proof}[Beweis von Satz~\ref{thm:wassersteinweakconvergence}]
		...
		kompakt ist. Da $\pi_k$ für jedes $k \in \N$ in $\hat{\Pi}(A, B)$ liegt, hat jede Teilfolge $(\pi_{k_l})_l$ von $(\pi_k)_k$ eine Teilfolge $(\pi_{k_{\tilde{l}}})_{\tilde{l}}$, 
		die schwach gegen ein $\tilde{\pi} \in \hat{\Pi}(A, B)$ konvergiert. Wegen Hilfssatz~\ref{lem:convimpliescouplingsconv} muss $\tilde{\pi}$ eine optimale Kopplung von $\mu$ und $\mu$ sein, also
		\[ \measureintx{X \times X}{d(x, y)^p}{\tilde{\pi}}{(x, y)} \; = \; W_p(\mu, \mu)^p \; = \; 0 \text{.} \]
		Daher gilt $\tilde{\pi}(\setcomp{(x, x)}{x \in X}) = 1$ und für alle $A, B \in \mathcal{B}(X)$ folgt $\tilde{\pi}(A \times B) = \mu(A \cap B)$, weshalb $\tilde{\pi}$ die 
