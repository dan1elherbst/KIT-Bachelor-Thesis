...
\begin{document}
	...
	\begin{Definition}[Kopplung]
		Seien $\X$ und $\Y$ topologische Räume sowie $A \subseteq \Probmeasures{\X}$ und $B \subseteq \Probmeasures{\Y}$. Wir definieren dann
	...
	\begin{proof}
		...
		für alle $\mu \in A$ und $\nu(L_{\varepsilon/2}) \geq 1 - \frac{\varepsilon}{2}$ für alle $\nu \in B$ gilt. Dann ist $K_{\varepsilon/2} \times L_{\varepsilon/2} \subseteq \X \times \Y$ auch kompakt und für jede Kopplung $\pi$ von $\mu$ und $\nu$ gilt
		\begin{align*}
	...
	\begin{Satz}
		...
		Seien $\X$ und $\Y$ polnische Räume, $\fct{c}{\X \times \Y}{[0, \infty)}$ eine stetige Kostenfunktion und seien $A \subseteq \Probmeasures{\X}$ und $B \subseteq \Probmeasures{\Y}$ kompakte Teilmengen.
		Dann ist 
	...
	\begin{proof}
		Siehe \cite[Remark 6.5]{Villani.2009}.
	\end{proof}
	...
	\begin{proof}
		...
		Für  $k \in \N$ sei nun $j \in \set{1, \dots, n}$ mit $W_1(\mu_j, \mu_k) < \varepsilon^2$. Da $\varepsilon\varphi$ lipschitzstetig mit Konstante $1$ ist, liefert die 
	...
	\begin{proof}[Beweis von Satz~\ref{satz:wassersteinmetrik_schwache_konvergenz}]
		...
		Da $A \defby \setcomp{\mu_k}{k \in \N} \cup \set{\mu}$ und $B \defby \set{\mu}$ kompakt sind, liefert Satz~\ref{satz:optimale_transportpläne_kompakt} die Kompaktheit von
		\[ \hat{\Pi}(A, B) \; \defby \; \setcomp{\pi \in \Pi(A, B)}{\pi \; \text{ist eine optimale Kopplung}} \; \subseteq \; \Probmeasures{\X \times \X} \text{.} \]
