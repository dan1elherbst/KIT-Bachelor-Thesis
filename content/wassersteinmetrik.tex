...
\begin{document}
	...
	- die sogenannten \emph{Wassersteinmetriken} - vorstellen, die die schwache Topologie metrisieren.
	...
	\begin{Satz}
		...
		Sei $(\X, d)$ ein polnischer metrischer Raum, wobei $d$ eine beschränkte Metrik sei, und sei $p \in [1, \infty)$. Dann wird $\Probmeasures{X}$ vollständig durch $W_p$ metrisiert. 
	\end{Satz}
	...
	\begin{proof}[Beweis von Satz~\ref{thm:wassersteinweakconvergence}]
		Wir zeigen zunächst, dass $\Probmeasures{X}$ von $W_p$ metrisiert wird. Da wir aus Satz~\ref{thm:finitemeasurespolish} bereits wissen, dass $\Probmeasures{X}$ metrisierbar ist, genügt es, die Äquivalenz
