...
\begin{document}
	...
	\begin{Bemerkung}
		...
		für einen beliebigen polnischen Raum $X$ beweisen. Diese ist im Allgemeinen gar nicht erfüllt, was man etwa am Beispiel
		$(l^\infty, \norm{\cdot}_\infty) = (\Bdcontfct{\N}, \norm{\cdot}_\infty)$ erkennen kann.
	\end{Bemerkung}
	...
	\begin{proof}
		...
		\[ A \defby \setcomp{l \in \Bdcontfct{X}^\ast}{\norm{l}_{\Bdcontfct{X}^\ast} \leq 1, \; l(\indfct_{X}) = 1, \; 
			\forall f \in \Bdcontfct{X} \text{ mit } f \geq 0 : l(f) \geq 0} \subseteq \Bdcontfct{X}^\ast \]
		und
		...
		Folgen-Homöomorphismus zwischen $\Probmeasures{X}$ mit der schwachen Konvergenz und $A \subseteq \Bdcontfct{X}^\ast$ 
		...
		Also ist auch $\Probmeasures{X}$ folgenkompakt. Da $X$ insbesondere vollständig und damit polnisch ist, 
		liefert Satz~\ref{thm:weakconvergencemetrizable} die Metrisierbarkeit von $\Probmeasures{X}$, womit
	...
	\begin{proof}
		...
		\begin{align*}
			\mu(K_\varepsilon) \; &\geq \; \mu(S_\varepsilon) \; = \; \mu\left(\bigcap_{k \in \N} \bigcup_{m=1}^{M_{\varepsilon / 2^k}} 
			                         ...
			                         \; = \; 1 - \mu\left(\bigcup_{k \in \N} \left(\bigcup_{m=1}^{M_{\varepsilon / 2^k}} 
			                         	B_{\varepsilon / 2^k}(x_m) \right)^{\!\! \mathsf{c}} \; \right) \\
