...
\begin{document}
	...
	\tobechanged{Beweise das bzw. überlege, ob man das noch machen sollte...}
	...
	\begin{proof}
		...
		die einen schwachen Grenzwert $\mu \in \Probmeasures{X}$ besitzt. Fixiere nun ein $M \in \N$. Dann ist $\bigcup_{m=1}^{M} B_\delta(x_m)$
		offen und daher liefert das Portmanteau-Theorem (Satz~\ref{thm:portmanteau})
		\[ \mu(\bigcup_{m=1}^{M} B_\delta(x_m)) \; \leq \; \liminf_{j \to \infty} \mu_{N_j}(\bigcup_{m=1}^{M} B_\delta(x_m)) \; \leq \; 1 - \delta \text{.} \label{4.10} \tag{4.10} \]
		Wegen $\bigcup_{m=1}^{\infty} B_\delta(x_m) = X$ und Maßstetigkeit von unten liefert uns \eqref{4.10} unmittelbar 
		\[ \mu(X) \; \leq \; 1 - \delta \; < \; 1 \text{,} \]
		was einen Widerspruch dazu darstellt, dass es sich bei $\mu$ um ein Wahrscheinlichkeitsmaß auf $X$ handelt. Also muss unsere Behauptung in \eqref{4.9} tatsächlich gelten.
		
		Sei nun ein $\varepsilon > 0$ gegeben. Dann setzen wir 
		\[ S_\varepsilon \; \defby \; \bigcap_{k \in \N} \bigcup_{m=1}^{M_{\varepsilon / 2^k}} B_{\varepsilon / 2^k}(x_m) \text{.} \]
		Offensichtlich ist $S_\varepsilon$ total beschränkt und damit auch $ K_\varepsilon \defby \overline{S_\varepsilon}$ kompakt (vgl. \cite[Satz 2.3.8]{Simon.2015}). Außerdem 
		gilt für alle $\mu \in A$
		\begin{align*}
			\mu(K_\varepsilon) \; &\geq \; \mu(S_\varepsilon) = \mu\left(\bigcap_{k \in \N} \bigcup_{m=1}^{M_{\varepsilon / 2^k}} B_{\varepsilon / 2^k}(x_m)\right)
			                         \; = \; 1 - \mu\left(\bigcup_{k \in \N} \left(\bigcup_{m=1}^{M_{\varepsilon / 2^k}} B_{\varepsilon / 2^k}(x_m) \right)^{\!\! \mathsf{c}} \; \right) \\
