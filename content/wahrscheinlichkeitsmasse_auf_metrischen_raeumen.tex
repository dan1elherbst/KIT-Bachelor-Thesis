...
\begin{document}
	...
	\begin{Bemerkung}[Schwach-$\ast$-Konvergenz]
		...
		\[\fctmap{\Phi}{\Finitemeasures{X}}{C_b(X)^\ast}{\mu}{\left[\fctmap{l_\mu}{\Bdcontfct{X}}{\R}{f}{\measureint{}{f}{\mu}}\right]} \text{,} \label{2.20} \tag{2.20}\]
		...
		Handelt es sich bei $X$ nun um einen metrischen Raum, so ist $\Phi$ sogar injektiv und damit ein \tobechanged{Folgen-Homöomorphismus} auf sein Bild:
		...
		$\Bdcontfct{X}^\ast$ mit der Schwach-$\ast$-Konvergenz auf die entsprechenden Eigenschaften von $\Finitemeasures{X}$ mit der Topologie der schwachen Konvergenz geschlossen wird. Ein Beispiel hierfür werden wir noch im Beweis des \tobechanged{Satzes von Prokhorov} sehen.
	\end{Bemerkung}
